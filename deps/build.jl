...
function main()
    ...
    open(ext, "w") do fh
        write(fh, """
            ...
            const libllvm_version = $(repr(llvm.version))
            const libllvm_path    = $(repr(llvm.path))
            const libllvm_mtime   = $(repr(llvm.mtime))
            const libllvm_system  = $(repr(use_system_llvm))
            const libllvm_targets = $(repr(llvm_targets))

            # LLVM extras library properties
            const libllvm_extra_path = $(repr(extras_path))

            # package properties
            const llvmjl_wrapper = $(repr(wrapper))
            const llvmjl_hash    = $(repr(llvmjl_hash))
            """)
    end
