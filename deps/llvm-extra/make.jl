...
push!(LDFLAGS, "-Wl,-rpath,$(LLVM_LIBRARY)")
...
reduce(vcat, CXXFLAGS, split(capture(`$(JULIA_BINARY) $(JULIA_CONFIG) --cflags`)))
reduce(vcat, LDFLAGS, split(capture(`$(JULIA_BINARY) $(JULIA_CONFIG) --ldflags`)))
if is_windows()
    push!(LDFLAGS, "-lssp")
end
...
SOURCES = filter(x -> ismatch(r".cpp$", x), dirwalk(pwd()))
...
dirwalk(path) = (filenames = String[]; dirwalk(filenames, path); filenames)
function dirwalk!(A, path::AbstractString)
...
function find_system_compiler()
    ...
    if is_windows()
        try
            ...
            error("Install GCC via `Pkg.add(\"WinRPM\"); using WinRPM; WinRPM.install(\"gcc\")`.")
        end
    end
    # see if `cc` exists
    try
