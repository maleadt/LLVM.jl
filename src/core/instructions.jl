...
Base.isempty(md::InstructionMetadataDict) =
  !convert(Bool, API.LLVMHasMetadata(md.inst))
...
istailcall(inst::CallBase) = convert(Bool, API.LLVMIsTailCall(inst))
...
isconditional(br::Instruction) = convert(Bool, API.LLVMIsConditional(br))
